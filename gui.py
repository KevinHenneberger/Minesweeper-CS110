...
colors["black"] = (0, 0, 0)
colors["gray1"] = (200, 200, 200)
colors["gray2"] = (175, 175, 175)
colors["blue1"] = (0, 0, 215)
colors["blue2"] = (0, 0, 255)
colors["blue3"] = (0, 0, 235)
colors["red1"] = (215, 0, 0)
colors["red2"] = (255, 0, 0)
colors["red3"] = (235, 0, 0)
colors["green1"] = (0, 135, 0)
colors["green2"] = (0, 175, 0)
colors["green3"] = (0, 155, 0)
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            if (board[row][col].isFlipped):
                pygame.draw.rect(window, colors["gray1"], [x, y, w, h])
                ...
                if (board[row][col].isFlagged):
                    pygame.draw.rect(window, colors["red1"], [x, y, w, h])
                    pygame.draw.rect(window, colors["red2"], [x + (w / 20), y + (h / 20), w - (w / 10), h - (h / 10)])
                    pygame.draw.rect(window, colors["red3"], [x + (w / 4), y + (h / 4), w - (w / 2), h - (h / 2)])
                elif (board[row][col].isMarked):
                    pygame.draw.rect(window, colors["green1"], [x, y, w, h])
                    pygame.draw.rect(window, colors["green2"], [x + (w / 20), y + (h / 20), w - (w / 10), h - (h / 10)])
                    pygame.draw.rect(window, colors["green3"], [x + (w / 4), y + (h / 4), w - (w / 2), h - (h / 2)])
                else:
                    pygame.draw.rect(window, colors["blue1"], [x, y, w, h])
                    pygame.draw.rect(window, colors["blue2"], [x + (w / 20), y + (h / 20), w - (w / 10), h - (h / 10)])
                    pygame.draw.rect(window, colors["blue3"], [x + (w / 4), y + (h / 4), w - (w / 2), h - (h / 2)])
...
def gameLoop():
    ...
    while (openWindow):
        ...
        if (screen == "main_menu_screen"):
            ...
            # GUI display
            window.fill(colors["gray1"])
            text("Minesweeper", window_width / 2, 50, "Impact", 48, colors["white"], "center")  
            displayButton(buttons["start"])
            displayButton(buttons["help"])
            displayButton(buttons["settings"])
            displayButton(buttons["high_scores"])
            text("Created by Kevin Henneberger, Adam Wiener,", window_width / 2, 425, "Impact", 16, colors["white"], "center")  
            text("Matt Aber, and Baptiste Saliba", window_width / 2, 450, "Impact", 16, colors["white"], "center")
        ...
        # help screen
            ...
            # GUI display
            window.fill(colors["gray1"])
            text("Help", window_width / 2, 50, "Impact", 48, colors["white"], "center")
            ...
            for i in range(len(instructions)):
                text(str(i + 1) + ") " + instructions[i], 35, i * 35 + 100, "Impact", 18, colors["white"], "left") 
        ...
        # settings screen
            ...
            # GUI display
            window.fill(colors["gray1"])
            text("Settings", window_width / 2, 50, "Impact", 48, colors["white"], "center")
        ...
        # high scores screen
            ...
            # GUI display
            window.fill(colors["gray1"])
            text("High Scores", window_width / 2, 50, "Impact", 48, colors["white"], "center")  
            ...
            for i in range(len(high_scores_list)):
                text(str(i + 1) + ") " + high_scores_list[i], 35, i * 35 + 100, "Impact", 18, colors["white"], "left") 
        ...
        # lose screen
            ...
            displayBoard(game_board.board, tile_width, tile_height, num_rows, num_cols)
            text("GAME OVER!", window_width / 2, 175, "Impact", 48, colors["white"], "center")  
        ...
        # win screen
            ...
            text("YOU WIN!", window_width / 2, 175, "Impact", 48, colors["white"], "center")  
            displayButton(buttons["play-again"])
