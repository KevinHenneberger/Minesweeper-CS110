...
* Fix the timer's display format
* Fix the score display.
mines with timers???
Timer counting down?
* documentation
* add quit button so that IDLE does not close when Close (on the window) is pressed. <--- I forgot what this means. Basically add a Quit button to the Main Menu screen.
* make it so the game doesn't not work as soon as it is opened
add the number of non-mine tiles remaining to be flipped
* add a back button on the game play screen
...
Change the name of num_mines_touches in fillBoard() in gameboard.py.
Is the final else statement in fillBoard() in gameboard.py unnecessary? Aren't the empty tiles preset to the value ' '?
What is clearEmptyCells() in gameboard.py saying?
Should the mines be flipped when the player wins the game?
In guaranteeEmptyCell() in gameboard.py, can a mine be placed back at its original location?
In guaranteeEmptyCell() in gameboard.py, after placeMine() is executed, are the values of the empty cells updated?
Why are there for loops in guaranteeEmptyCell() in gameboard.py? Can't they be eliminated?
* Change the text on the Play Again button when the game ends.
...
Rename the play_again_l and play_again_w buttons so that their names are clearer.
Why do the two play_again buttons do the same thing? Can one be eliminated?
* Rename the play_again buttons main_menu, and add a single play_again button that restarts the game without forcing the player to go to the main menu.
Rename the back buttons as main_menu buttons?
Change the names of the colors to make them clearer. Change 1, 2, and 3, to dark, light, and medium, respectively. Change gray to grey.
Black is not used anywhere in the game, and should be removed from the color dictionary in __init__() in gui.py.
* Update the Help or Settings screens to tell the player how many mines are in each level of difficulty. Easy has 10. Medium has 24. Hard has 40. Also tell the sizes of the boards.
What is the purpose of True in t = f.render(txt, True, color) in text() in gui.py?
What does the alignment part of text() in gui.py mean?
In gui.py, what sets the gameboard's location?
In gui.py, what are tile_w and tile_h?
* In gui.py, in mainMenu(), what is the event type pygame.QUIT? How does the player perform this event?
* Change Start to Play Game.
Rename mines_remaining in __init__() in gui.py.
Should playGameScreen() in gui.py, in its if statement on left clicks, include a condition that the tile be unflipped?
Change Lose screen to Loss screen.
...
* In gui.py, tile_w and tile_h are defined twice. Once in __init__(), and once in gameLoop().
* In gui.py, in gameLoop(), what do pygame.quit() and quit() do? If both are removed, the program does not close. If only quit() is removed, the program closes effortlessly. If only pygame.quit() is removed, trying to close the program requires special permission to kill the program, because the program is still running. Removing neither has the same effect.
What does count do?
In gui.py, is mousePosition = pygame.mouse.get_pos() a list or a tuple?
Two different back buttons are constructed in highScoreScreen(), in gui.py, even though only one is shown: the High Scores back button and the Help back button.
Also, change the name of highScoreScreen() to highScoresScreen() in gui.py.
Finish the documentation for playGameScreen in gui.py.
...
In winScreen(), in gui.py, finish the documentation for the keyboard presses.
